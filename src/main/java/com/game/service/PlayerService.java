...
public class PlayerService{
    ...
    private static final int MAX_SIZE_NAME = 12;
    private static final int MAX_SIZE_TITLE = 30;
    ...
    public Page<Player> findPlayers(Specification<Player> specification, Pageable pageable) {
    ...
    public Player updatePlayer(Long id, Player player) {
        Player newPlayer = getPlayerId(id);
    ...
    public void checkName(String name) {
        if(name==null || name.isEmpty() || name.length()> MAX_SIZE_NAME)
            throw new BadRequestException("Name is invalid");
    }

    public void checkTitle(String title) {
        if(title.length()> MAX_SIZE_TITLE || title==null || title.isEmpty())
